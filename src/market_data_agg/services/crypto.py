"""Crypto service: quote, overview, history, refresh, stream with unified error mapping."""
...
class CryptoService:
    """Thin service over crypto provider; maps provider errors to HTTP."""

    def __init__(self, provider: MarketProviderABC) -> None:
        self._provider = provider

    async def get_quote(self, symbol: str) -> MarketQuote:
        """Get current quote for a crypto. Raises HTTPException on provider errors."""
        try:
            return await self._provider.get_quote(symbol)
        except Exception as e:
            raise_provider_http(e, resource_name="Crypto", symbol=symbol, api_name="CoinGecko")

    async def get_overview_quotes(self) -> list[MarketQuote]:
        """Get overview (top by market cap) crypto quotes. Raises HTTPException on provider errors."""
        try:
            return await self._provider.get_overview_quotes()
        except Exception as e:
            raise_provider_http(e, resource_name="Crypto", api_name="CoinGecko")

    async def get_history(self, symbol: str, days: int) -> list[MarketQuote]:
        """Get historical quotes for a crypto. Raises HTTPException on provider errors."""
        end = datetime.utcnow()
        start = end - timedelta(days=days)
        try:
            return await self._provider.get_history(symbol, start, end)
        except Exception as e:
            raise_provider_http(
                e,
                resource_name="History for crypto",
                symbol=symbol,
                api_name="CoinGecko",
            )

    async def refresh(self) -> None:
        """Force refresh the crypto provider. Raises HTTPException on failure."""
        try:
    ...
    async def handle_websocket_stream(
        ...
        symbol_list = parse_symbols_param(websocket.query_params, normalizer=str.lower)
