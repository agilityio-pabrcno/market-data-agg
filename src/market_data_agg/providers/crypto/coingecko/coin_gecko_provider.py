...
class CoinGeckoProvider(MarketProviderABC):
    ...
    async def _fetch_stream_batch(self, syms: list[str]) -> list[MarketQuote]:
        ...
        data = response.json()
        return [
            self._quote_from_simple_price(cid, data[cid])
            for cid in ids
            if cid in data and (row := data[cid]) and row.get("usd") is not None
        ]
    ...
    def _quote_from_simple_price(self, coin_id: str, data: dict) -> MarketQuote:
        ...
        return MarketQuote(
            ...
            volume=round2(float(vol)) if vol is not None else None,
